...
package body BigNumPkg is
   ...
   -- Precondition: n/a
   -- Postcondition: Returns a new string that represents a
   -- bignum with leading zeros removed
   ----------------------------------------------------------
   function toString(X: BigNum) return String is
     s : String(1..Size);
     len : Integer := 0;
     got_non_zero : Boolean := false;
   begin
      for i in 1 .. Size loop
        if Character'Val(X(i)) /= '0' then
          got_non_zero := true;
        end if;

        if got_non_zero then
          s(1+len) := Character'Val(X(i));
          len := len + 1;
        end if;
      end loop;
      return s(1..len);
