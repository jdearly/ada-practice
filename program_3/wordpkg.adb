...
package body WordPkg is
	...
	function New_Word(Item : String) return Word is
		A_Word : word;
	begin
		A_Word.CalcElement(1..Item'Length) := Item;
	...
	----------------------------------------------------------
	-- Purpose: Converts Words to strings.
	-- Parameters: w
	-- Precondition: w is a Word type
	-- Postcondition: Returns word converted to String
	----------------------------------------------------------
	function to_string (w : word) return String is
		s : String(1..2); -- will need to adjust (unbounded)
	begin
	...
	begin
		return X.Length = Y.Length and then
			X.CalcElement(1..X.Length) = Y.CalcElement(1..Y.Length);
	end "=";

	function "<="  (X, Y : Word) return Boolean is
	begin
		For I in 1..Natural'Min (X.Length, Y.Length)  loop
			if X.CalcElement(I) < Y.CalcElement(I) then
				return True;
			elsif X.CalcElement(I) > Y.CalcElement(I) then
				return False;
			end if;
		end loop;
		...
		if Another_Word (File) then
			loop
				...
				if not TooLong then
					...
					Item.CalcElement(Item.Length) := C;
				end if;
			end loop;
			if TooLong then
				raise WordTooLong;
